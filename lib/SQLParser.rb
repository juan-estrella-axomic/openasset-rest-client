...
class SQLParser
    ...
    def parse_query(original_query,end_point)
        ...
        VALID_COMPARISON_OPERATORS.each do |op|
            # Protect against "in" being found inside a field name
            # capture indecies of all occurences of the operator
            regex = op.eql?('in') ? /\s+#{op}\s+/ : /#{op}/
            while index = str =~ regex
                str.sub!(regex,'~') # replace current occurence the operator so having one >= doesn't show as having >= > =
